...
function getEslintVersion() {
  var _a;
  try {
    const versionOutput = (0, import_child_process.execSync)("npx eslint --version", {
      encoding: "utf-8"
    });
    ...
    if (import_fs.default.existsSync(packageJsonPath)) {
      ...
      if (eslintVersion) {
        return ((_a = eslintVersion.match(/\d+/)) == null ? void 0 : _a[0]) ?? null;
      }
    }
    return null;
  }
...
function checkPackageJsonConfig(keys) {
  const packageJsonPath = import_path.default.join(process.cwd(), "package.json");
  if (import_fs.default.existsSync(packageJsonPath)) {
    const packageJson = JSON.parse(import_fs.default.readFileSync(packageJsonPath, "utf-8"));
...
function copyConfigFile(configDir, fileName) {
  const srcPath = import_path.default.join(configDir, fileName);
  const packageJsonPath = import_path.default.join(process.cwd(), "package.json");
  if (import_fs.default.existsSync(packageJsonPath)) {
    const packageJson = JSON.parse(import_fs.default.readFileSync(packageJsonPath, "utf-8"));
  ...
  let content = import_fs.default.readFileSync(srcPath, "utf-8");
  import_fs.default.writeFileSync(destPath, content);
  console.log(green(`\u2728 ${fileName} \u5DF2\u521B\u5EFA`));
}
function mergePackageJson(dependencies, devDependencies) {
  const packageJsonPath = import_path.default.join(process.cwd(), "package.json");
  const packageJson = import_fs.default.existsSync(packageJsonPath) ? JSON.parse(import_fs.default.readFileSync(packageJsonPath, "utf-8")) : {};
...
async function installWithPackageManager(packageManager, response) {
  ...
  if (isHuskyIntegrated()) {
    await withSpinner("\u521D\u59CB\u5316 Husky", async () => {
      (0, import_child_process.execSync)("npx husky init", { stdio: "inherit" });
      if (response.husky) {
        console.log(green("\n\u{1F4DD} \u914D\u7F6E pre-commit \u94A9\u5B50..."));
        const preCommitPath = import_path.default.join(".husky", "pre-commit");
        import_fs.default.writeFileSync(preCommitPath, "npx lint-staged\n");
        import_fs.default.chmodSync(preCommitPath, "755");
        console.log(green("\u2705 pre-commit \u94A9\u5B50\u5DF2\u914D\u7F6E"));
      }
      if (response.commitlint) {
        console.log(green("\n\u{1F4DD} \u914D\u7F6E commit-msg \u94A9\u5B50..."));
        const commitMsgPath = import_path.default.join(".husky", "commit-msg");
        import_fs.default.writeFileSync(
          commitMsgPath,
          "npx --no-install commitlint --edit $1\n"
        );
        import_fs.default.chmodSync(commitMsgPath, "755");
...
function checkPackageJsonForDependencies(dependencies) {
  const packageJsonPath = import_path.default.join(process.cwd(), "package.json");
  if (!import_fs.default.existsSync(packageJsonPath)) {
    return false;
  }
...
function isCzGitIntegrated() {
  const czGitDependencies = ["cz-git", "czg"];
  return checkPackageJsonForDependencies(czGitDependencies);
}
function checkExistingConfig(altForms) {
  for (const form of altForms) {
    const configPath = import_path.default.join(process.cwd(), form);
...
async function mergeConfigFile(configDir, existingConfigPath, configKey) {
  if (existingConfigPath === "package.json") {
    await withSpinner(`\u5408\u5E76 ${configKey} \u914D\u7F6E`, async () => {
      const packageJsonPath = import_path.default.join(process.cwd(), "package.json");
      const srcPath = import_path.default.join(configDir, import_path.default.basename(existingConfigPath));
      const packageJson = JSON.parse(import_fs.default.readFileSync(packageJsonPath, "utf-8"));
      const newConfig = JSON.parse(import_fs.default.readFileSync(srcPath, "utf-8"));
...
async function withSpinner(message, callback) {
...
async function setupProject() {
  ...
  const questions = [
    ...
    },
    {
      type: "confirm",
      name: "eslint",
      message: bold(blue("\u662F\u5426\u9700\u8981\u96C6\u6210 Eslint?")) + " (Eslint \u662F\u4E00\u4E2A\u4EE3\u7801\u8D28\u91CF\u68C0\u67E5\u5DE5\u5177)",
      initial: !isEslintIntegrated()
    },
    {
      type: "confirm",
      name: "prettier",
      message: bold(blue("\u662F\u5426\u9700\u8981\u96C6\u6210 Prettier?")) + " (Prettier \u662F\u4E00\u4E2A\u4EE3\u7801\u683C\u5F0F\u5316\u5DE5\u5177)",
      initial: !isPrettierIntegrated()
    },
    {
      type: "confirm",
      name: "husky",
      message: bold(blue("\u662F\u5426\u9700\u8981\u96C6\u6210 Husky + Lint-staged?")) + " (Husky \u662F\u4E00\u4E2A\u7528\u4E8E\u7BA1\u7406 Git hooks \u7684\u5DE5\u5177)",
      initial: !isHuskyIntegrated()
    },
    {
      type: "confirm",
      name: "commitlint",
      message: bold(blue("\u662F\u5426\u9700\u8981\u96C6\u6210 Commitlint?")) + " (Commitlint \u662F\u4E00\u4E2A\u7528\u4E8E\u6821\u9A8C\u63D0\u4EA4\u4FE1\u606F\u7684\u5DE5\u5177)",
      initial: !isCommitlintIntegrated()
    },
    {
      type: "confirm",
      name: "czGit",
      message: bold(blue("\u662F\u5426\u9700\u8981\u96C6\u6210 cz-git?")) + " (cz-git \u662F\u4E00\u4E2A\u8F85\u52A9\u751F\u6210\u6807\u51C6\u5316\u89C4\u8303\u5316\u7684 commit message \u7684\u5DE5\u5177)",
  ...
  try {
    const baseConfigDir = import_path2.default.join(
      __dirname,
      "..",
      "presetConfig",
      response.language
    ...
    const mergeDependenciesFromPreset = (configDir) => {
      const presetPackageJsonPath = import_path2.default.join(configDir, "package.json");
      if (import_fs2.default.existsSync(presetPackageJsonPath)) {
        const presetPackageJson = JSON.parse(
          import_fs2.default.readFileSync(presetPackageJsonPath, "utf-8")
        );
    ...
    if (response.eslint) {
      ...
      const eslintConfigDir = import_path2.default.join(baseConfigDir, "eslint");
      const altForms = isEslint9OrAbove ? ["eslint.config.js", "eslint.config.mjs", "eslint.config.cjs"] : [
        ".eslintrc.js",
        ".eslintrc.cjs",
        ".eslintrc.yaml",
        ".eslintrc.yml",
    ...
    if (response.prettier) {
      ...
      const prettierConfigDir = import_path2.default.join(baseConfigDir, "prettier");
      const altForms = [
        ".prettierrc",
        ".prettierrc.js",
        ".prettierrc.mjs",
    ...
    if (response.husky) {
      ...
      const huskyConfigDir = import_path2.default.join(baseConfigDir, "..", "base", "husky");
      mergeDependenciesFromPreset(huskyConfigDir);
      const lintStagedConfig = {};
      const fileExtensions = response.language === "typescript" ? ["*.{ts,tsx}"] : ["*.{js,jsx}"];
      ...
      if (response.eslint && response.prettier) {
        ...
        lintStagedConfig[`*.{${[...fileExtensions[0].slice(2), ...prettierExtensions].join(",")}}`] = ["prettier --write"];
      }
      ...
      import_fs2.default.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));
      console.log(green("\u2728 \u5DF2\u6DFB\u52A0 lint-staged \u914D\u7F6E"));
    }
    if (response.czGit) {
      spinner.text = bold("\u914D\u7F6E cz-git...");
      const czGitConfigDir = import_path2.default.join(baseConfigDir, "..", "base", "cz-git");
    ...
    if (response.commitlint) {
      ...
      const commitlintConfigDir = import_path2.default.join(baseConfigDir, "commitlint");
      const altForms = [
        ".commitlintrc",
        ".commitlintrc.json",
        ".commitlintrc.yaml",
